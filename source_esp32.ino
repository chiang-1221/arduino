...
#include <Adafruit_PWMServoDriver.h> //使用 Arduino PWM的接腳來控制伺服馬達， 呼叫伺服驅動程式函數
...
const float bone_length = 105; // 單位 mm
...
int state = 0; //> 表示類型，(0) 空閒，(1) 小跑，(2) 偏航，(3) 俯仰滾動，(4) 物體檢測
float _period = 10.0; //> 每秒的步數

datatypes::Rotator _sRotation; //> 儲存了body的相對旋轉
...
void loop() {
  ...
  handle_hardware();
  handle_kinematics(_direction, turn, height, _period); //(位置,旋轉方向,腿部伸展,每秒步數)
...
bool _tb = false; //判斷狀態是否執行，若為1則執行以下程式
float stick_min = 6.f; 
float lx, ly, rx, ry;
void handle_input() {
  if (PS4.isConnected()) {
    ...
    if (abs(lx) > stick_min) { //> 取絕對值並檢查搖杆位置是否超出範圍
      float x0 = lx - stick_min * sign(lx); //> 減去盲區
      if (state == 1) {
    ...
    if (abs(ly) > stick_min) { //> 檢查搖杆位置是否超出範圍
      float y0 = ly - stick_min * sign(ly); //> 減去盲區
      if (state == 1) {
    ...
    if (abs(rx) > stick_min) { //> 檢查搖杆位置是否超出範圍
      float x1 = rx - stick_min * sign(rx); //> 減去盲區
      if (state == 1)
        turn = x1 / 16.f;
      else if (state != 4)
        turn = x1;
    ...
    if (abs(ry) > stick_min) { //> 檢查搖杆位置是否超出範圍
      float y1 = ry - stick_min * sign(ry); //> 減去盲區
      height = y1;
    } else height = 0;
  }
